...
BOOL CLedCtrlServerDlg::OnInitDialog()
{
	...
	if (!bResult)
	{
		...
		Tips.AddString(_T("[OnInitDialog]--获取配置文件中的平台地址字段失败！"));
		return FALSE;
	}
	DWORD dwThreadId = 0;
	...
	Tips.AddString(_T("[OnInitDialog]--CreateThread ReSignThreadProc"));
...
BOOL CLedCtrlServerDlg::LoadDeviceConfig()
{
	...
	Tips.AddString(_T("[LoadDeviceConfig]--CreateThread RMQThreadProc"));
	Tips.AddString(_T("[LoadDeviceConfig]--CreateThread DeviceStatusThreadProc"));
	Tips.AddString(_T("[LoadDeviceConfig]--CreateThread MMPDeviceStatusThreadProc"));
	return TRUE;
}
		...
		if (vct[0].CompareNoCase("PUBLISH") == 0)
		{
			if (vct[1].CompareNoCase(g_Config.m_strDeviceCode) == 0)
			{
				...
				else
				{
					...
					{
						Tips.AddString(_T("[RMQ_DealCustomMsg]--调用平台节目发布接口失败！"));
		...
		if (dwResult == WAIT_OBJECT_0)
		{
			...
			if (FALSE == bRet)
			{
				...
				if (icount == 2)
				{
					...
					Tips.AddString(_T("[ReSignThreadContent]--签到失败..."));
				}
			}
			else
			{
...
void CLedCtrlServerDlg::ThreadPoolPublishPrgm(CString strHttpAddr,CString strPrgmID, CString strPublicUser, CString strPublicTime, CString strDeviceModel, CString strCardAddr, CString strDeviceID, CString strIP, CString strPort, CString strLedType, CString strControlCardType, CString strHeight, CString strWidth, CString strPrgmType, CString strPrgmFontSize, CString strPrgmContent, CString strPrfmIndex, CString strPrgmEffect, CString strPrgmStayTime, CString strPrgmPlaySpeed)
{
	...
	//		if (IsSuccess)
	//		{
	//			Tips.AddString(_T("[ThreadPoolPublishPrgm]--诣阔控制卡节目下发成功"));
	...
	//		if (IsSuccess)
	//		{
	//			Tips.AddString(_T("[ThreadPoolPublishPrgm]--诣阔控制卡节目发布失败"));
	...
	//		if (IsSuccess)
	//		{
	//			Tips.AddString(_T("[ThreadPoolPublishPrgm]--励研控制卡节目下发成功"));
	...
	//		if (IsSuccess)
	//		{
	//			Tips.AddString(_T("[ThreadPoolPublishPrgm]--励研卡节目下发失败!"));
	//			LOG_DD(LOGTYPE_DEBUG, PUBLISHRECOARD, "[ThreadPoolPublishPrgm]--励研卡节目下发失败[%s][%s]", t_strIP, t_strPrgmID);
	//		}
	//		else
	//		{
	//			Tips.AddString(_T("[ThreadPoolPublishPrgm]--励研卡下发失败,上送节目下发结果失败"));
	...
	//		if (IsSuccess)
	//		{
	//			Tips.AddString(_T("[ThreadPoolPublishPrgm]--仰邦卡下发节目成功"));
	//			LOG_DD(LOGTYPE_DEBUG, PUBLISHRECOARD, "[ThreadPoolPublishPrgm]--仰邦卡下发节目成功[%s][%s]", t_strIP, t_strPrgmID);
	//		}
	//		else
	//		{
	//			Tips.AddString(_T("[ThreadPoolPublishPrgm]--仰邦卡节目下发成功"));
	...
	//		if (IsSuccess)
	//		{
	//			Tips.AddString(_T("[ThreadPoolPublishPrgm]--仰邦卡节目下发失败!"));
	...
	//		strBody.Format("\"itemtype\":\"3\",\"status\":\"1\",\"mmpdeviceid\":\"%s\",\"releaseuser\":\"%s\",\"releasetime\":\"%s\",\"itemId\":\"%s\"", t_strDeviceID, t_strPublicUser, t_strPublicTime, t_strPrgmID);
	//		IsSuccess = m_toolTrade.UpLoadProGrameStatus(t_strHttpAddr, strBody);
	//		Tips.AddString(_T("[ThreadPoolPublishPrgm]--视展A卡打开设备失败!"));
	...
	//		if (IsSuccess)
	//		{
	//			Tips.AddString(_T("[ThreadPoolPublishPrgm]--视展A卡下发节目成功"));
	...
	//		strBody.Format("\"itemtype\":\"3\",\"status\":\"1\",\"mmpdeviceid\":\"%s\",\"releaseuser\":\"%s\",\"releasetime\":\"%s\",\"itemId\":\"%s\"", t_strDeviceID, t_strPublicUser, t_strPublicTime, t_strPrgmID);
	//		IsSuccess = m_toolTrade.UpLoadProGrameStatus(t_strHttpAddr, strBody);
	//		Tips.AddString(_T("[ThreadPoolPublishPrgm]--视展D卡发布节目失败!"));
	...
	//		if (IsSuccess)
	//		{
	//			Tips.AddString(_T("[ThreadPoolPublishPrgm]--视展D卡下发节目成功"));
	...
	//	if (IsSuccess)
	//	{
	//		Tips.AddString(_T("[ThreadPoolPublishPrgm]--下发失败，不支持此种控制卡"));
	//		LOG_DD(LOGTYPE_DEBUG, PUBLISHRECOARD, "[ThreadPoolPublishPrgm]--不支持此种控制卡[%s]", t_strDeviceModel);
	//	}
	//	else
	//	{
	//		Tips.AddString(_T("[ThreadPoolPublishPrgm]--下发失败,上送节目下发结果失败"));
...
void CLedCtrlServerDlg::DealPublicPrgm(CString strJson,CString strUrl,CString strUser,CString strTime,CString strID)
{
		...
		if (jdata.is_array())
		{
			for (json::iterator it = jdata.begin(); it != jdata.end(); ++it)
			{
				...
				std::thread t2([=](){
					...
					CString strBody = "";
					BOOL IsSuccess = FALSE;
					if (strDeviceModel == "EQ3002-I" ||strDeviceModel == "EQ3002-II" ||strDeviceModel == "EQ3002-III" ||strDeviceModel == "EQ2008-I/II" ||strDeviceModel == "EQ2010-I" ||strDeviceModel == "EQ2008 - IE" ||strDeviceModel == "EQ2011" ||strDeviceModel == "EQ2012" ||strDeviceModel == "EQ2008-M" ||strDeviceModel == "EQ2013" ||strDeviceModel == "EQ2023" ||strDeviceModel == "EQ2033")
					{
						CString strFilepath = GetFullPath("EQ2008_Dll_Set.ini");
						strFilepath.Replace("/", "\\");
						IsSuccess = EQSendPrgm(strFilepath.GetBuffer(0),strLedType,strDeviceModel,strIP,strPrgmEffect,strPrgmContent,strHeight,strWidth,strPrgmFontSize,strPrgmPlaySpeed,strPrgmStayTime);
						if (IsSuccess)
						{
							strBody = "";
							...
							if (IsSuccess)
							{
								Tips.AddString(_T("[DealPublicPrgm]--诣阔控制卡节目下发成功"));
						...
						else
						{
							...
							if (IsSuccess)
							{
								Tips.AddString(_T("[DealPublicPrgm]--诣阔控制卡节目发布失败"));
					...
					else if (strDeviceModel == "CL3000" || strDeviceModel == "SCL2008")
					{
						...
						if (IsSuccess)
						{
							...
							if (IsSuccess)
							{
								Tips.AddString(_T("[DealPublicPrgm]--励研控制卡节目下发成功"));
						...
						else
						{
							...
							if (IsSuccess)
							{
								Tips.AddString(_T("[DealPublicPrgm]--励研卡节目下发失败!"));
								LOG_DD(LOGTYPE_DEBUG, PUBLISHRECOARD, "[DealPublicPrgm]--励研卡节目下发失败[%s][%s]",strIP,strPrgmID);
							}
							else
							{
								Tips.AddString(_T("[DealPublicPrgm]--励研卡下发失败,上送节目下发结果失败"));
					...
					{
						IsSuccess = onbon.PublishPrograms(strIP,strPort,strLedType,strDeviceModel,strPrgmContent, atoi(strWidth), atoi(strHeight), atoi(strPrgmFontSize), atoi(strPrgmEffect), atoi(strPrgmPlaySpeed), atoi(strPrgmStayTime));
						if (IsSuccess)
						{
							strBody = "";
							strBody.Format("\"itemtype\":\"3\",\"status\":\"0\",\"mmpdeviceid\":\"%s\",\"releaseuser\":\"%s\",\"releasetime\":\"%s\",\"itemId\":\"%s\"", strDeviceID,strPublicUser,strPublicTime, strPrgmID);
							...
							if (IsSuccess)
							{
								Tips.AddString(_T("[DealPublicPrgm]--仰邦卡下发节目成功"));
								LOG_DD(LOGTYPE_DEBUG, PUBLISHRECOARD, "[DealPublicPrgm]--仰邦卡下发节目成功[%s][%s]",strIP,strPrgmID);
							}
							else
							{
								Tips.AddString(_T("[DealPublicPrgm]--仰邦卡节目下发成功"));
						...
						else
						{
							...
							if (IsSuccess)
							{
								Tips.AddString(_T("[DealPublicPrgm]--仰邦卡节目下发失败!"));
					...
					else if (strDeviceModel == "VSA")
					{
						...
						if (bRet)
						{
							...
							strBody.Format("\"itemtype\":\"3\",\"status\":\"1\",\"mmpdeviceid\":\"%s\",\"releaseuser\":\"%s\",\"releasetime\":\"%s\",\"itemId\":\"%s\"", strDeviceID,strPublicUser,strPublicTime,strPrgmID);
							IsSuccess = m_toolTrade.UpLoadProGrameStatus(strHttpAddr, strBody);
							Tips.AddString(_T("[DealPublicPrgm]--视展A卡打开设备失败!"));
							LOG_DD(LOGTYPE_DEBUG, PUBLISHRECOARD, "[DealPublicPrgm]--视展A卡节目下发失败[%s][%s]", strIP,strPrgmID);
						}
						else
						{
							...
							if (IsSuccess)
							{
								Tips.AddString(_T("[DealPublicPrgm]--视展A卡下发节目成功"));
								LOG_DD(LOGTYPE_DEBUG, PUBLISHRECOARD, "[DealPublicPrgm]--视展A卡下发节目成功[%s][%s]",strIP, strPrgmID);
							}
						}
					}
					else if (strDeviceModel == "VSD")
					{
						int nRet = vsd.LED_VSDSendPrgm(strIP, strPort, strLedType,atoi(strWidth), atoi(strHeight), strPrgmContent,strPrgmFontSize, atoi(strPrgmEffect), atoi(strPrgmPlaySpeed));
						if (nRet != 0)
						{
							strBody = "";
							strBody.Format("\"itemtype\":\"3\",\"status\":\"1\",\"mmpdeviceid\":\"%s\",\"releaseuser\":\"%s\",\"releasetime\":\"%s\",\"itemId\":\"%s\"", strDeviceID,strPublicUser,strPublicTime,strPrgmID);
							IsSuccess = m_toolTrade.UpLoadProGrameStatus(strHttpAddr, strBody);
							Tips.AddString(_T("[DealPublicPrgm]--视展D卡发布节目失败!"));
							LOG_DD(LOGTYPE_DEBUG, PUBLISHRECOARD, "[DealPublicPrgm]--视展D卡节目下发失败[%s][%s]", strIP, strPrgmID);
						}
						else
						{
							strBody = "";
							...
							if (IsSuccess)
							{
								Tips.AddString(_T("[DealPublicPrgm]--视展D卡下发节目成功"));
								LOG_DD(LOGTYPE_DEBUG, PUBLISHRECOARD, "[DealPublicPrgm]--视展D卡下发节目成功[%s][%s]", strIP, strPrgmID);
							}
						}
					}
					else
					{
						...
						if (IsSuccess)
						{
							Tips.AddString(_T("[DealPublicPrgm]--下发失败，不支持此种控制卡"));
							LOG_DD(LOGTYPE_DEBUG, PUBLISHRECOARD, "[DealPublicPrgm]--不支持此种控制卡[%s]",strDeviceModel);
						}
						else
						{
							Tips.AddString(_T("[DealPublicPrgm]--下发失败,上送节目下发结果失败"));
							LOG_DD(LOGTYPE_DEBUG, PUBLISHRECOARD, "[DealPublicPrgm]--上送节目下发结果失败[%s]",strPrgmID);
						}
					}
	...
	else
	{
		...
		if (IsSuccess)
		{
			Tips.AddString(_T("[DealPublicPrgm]--解析json的body字段失败，上送下发状态成功"));
			LOG_DD(LOGTYPE_DEBUG, PUBLISHRECOARD, "[DealPublicPrgm]--解析json的body字段失败,上送下发状态成功");
		}
		else
		{
			Tips.AddString(_T("[DealPublicPrgm]--解析json的body字段失败，上送下发状态失败"));
...
BOOL CLedCtrlServerDlg::EQSendPrgm(const char* pFile, CString strLedType, CString strDeviceModel, CString strIP, CString strPrgmEffect, CString strPrgmContent, CString strHeight, CString strWidth, CString strPrgmFontSize, CString strPrgmPlaySpeed, CString strPrgmStayTime)
{
...
BOOL CLedCtrlServerDlg::CLSendPrgm(CString strLedType, CString strDeviceModel, CString strIP, CString strPort, CString strPrgmContent, CString strHeight, CString strWidth, CString strPrgmFontSize, CString strPrgmPlaySpeed)
{
